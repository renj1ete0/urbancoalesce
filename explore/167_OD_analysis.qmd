...
```{r}
gen_od_timing_SZ <- function(input_OD, sf_bs, timing){
  OD_TEST_DIR1 <- input_OD %>% filter(DAY_TYPE == "WEEKDAY" & TIME_PER_HOUR == timing)
...
tmap_plot_sz(sf_BS_167_DIR_1_MPSZ, temp_sf_sz_intra_flows)
temp_sf_sz_intra_flows %>% arrange(desc(SZ_TRIPS))

plot_trip_summary(temp_sf_sz_intra_flows)
```

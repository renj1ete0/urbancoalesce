...
There are inaccuracies in the normalisation of transit data to extract bus service 167's data, the code will be rewritten to fix issues in missing counts arising from:
1. Invalid bus service data - last bus is earlier than first bus on some services
2. Invalid assumptions - eg. Some AM Peak bus services run until outside AM Peak, however, AM Off peak schedule is not allocated, leading to invalid counts

Proposed workarounds:
- Convert all bus services by preprocessing into a dataframe for bus per hour beofre using the bph data frame to normalise - leads to better performance as well
...
tmap_animation(tm_objs,filename = "D:/167_OD_analysis/test.gif", width=2500, height=1500, dpi=200, outer.margins = 0)
```
...
```{r}
mpsz_pln_area <- st_read(dsn = "D:/167_OD_analysis/",

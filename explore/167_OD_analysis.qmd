...
```{r}
pacman::p_load(dplyr, readr, sf, tidyverse, tmap, sfdep, ggpubr, Metrics, ggplot2, plotly, spdep, rjson, od, gifski)
```
...
```{r}
mpsz_2014_sf <- st_read("data/167_OD_analysis/mpsz_2014/MP14_SUBZONE_WEB_PL.shp")
```
...
while (i_time < 24){
  ...
  #tm_shape(sf_BS_167_DIR_2) +
  #  tm_dots(col = "blue", scale = 2) +
  tm_shape(mpsz_2014_sf, bbox = c(22000, 27000, 34000, 49000)) +
    tm_polygons(alpha=0) +
  tm_shape(temp_sf) +
